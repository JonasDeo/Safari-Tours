...
const NavItem = ({
  ...
  return (
    ...
    >
      ...
      {/* Icon — show dot on collapsed if there's a pending badge */}
      <div className="relative flex-shrink-0">
        <item.icon className="w-4 h-4" />
        {collapsed && !mobile && badge && badge > 0 ? (
      ...
      <nav className="flex-1 px-2 py-4 space-y-0.5 overflow-y-auto admin-scrollbar">
        ...
        {NAV_MAIN.map(item => (
          <NavItem
            ...
            collapsed={collapsed}
            mobile={mobile}
            badge={item.href === "/admin/quotes" ? pendingQuotes : undefined}
...
const AdminLayout = () => {
  ...
  // Fetch pending quote count once on mount — keeps sidebar badge live
  useEffect(() => {
    adminApi.getStats()
      .then((data: any) => setPendingQuotes(data?.quotes?.pending ?? 0))
      .catch(() => {});
  }, []);
