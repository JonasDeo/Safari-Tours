...
const RadioSelect = ({ options, selected, onSelect, errors }: RadioSelectProps) => {
  const errorMsg = errors?.accommodation;
        ...
        {options.map((opt) => {
          ...
          return (
            <button
              ...
              className={`flex items-start gap-3 p-3.5 sm:p-4 rounded-2xl border text-left
            ...
            >
              ...
              <span>
                ...
                {opt.sub && (
                  <span className="block text-xs text-muted-foreground mt-0.5 font-body leading-relaxed">
                    {opt.sub}
                  </span>
                )}
              </span>
